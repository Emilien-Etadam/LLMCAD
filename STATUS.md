...
| `cadquery/server.py` | Pool global, `atexit` + **handlers `SIGTERM` / `SIGINT`** → `pool.shutdown()`, routes sur `pool.execute`, `/health` JSON avec `workers_alive` / `workers_total`, `threaded=True`. |
...
1. **Segfault / crash natif** : en laboratoire, deux observations peuvent coexister — (a) lecture sur stdout d’un worker qui a planté peut tarder selon l’OS / WSL ; (b) le pool remplace le worker et **`/health`** peut déjà montrer `workers_alive == workers_total` en ~**1 s** après un `ctypes.string_at(0)` (mesure agent 2026-04-30). Le **timeout** wall-clock reste le garde-fou.
2. **`pool.execute()` et « zombie »** : on ne remet en idle qu’un handle après une ligne JSON complète ou un chemin d’erreur ; si `poll() != None`, on **remplace** le worker avant de rendre la main. Si le noyau laisse le PID « vivant » un peu alors que le processus est cassé (cas extrême / WSL), la **requête suivante** peut échouer sur pipe invalide → recycle identique à un worker mort ; on ne réutilise pas un Popen dont `poll()` annonce la mort.
3. **Réponses `/preview` volumineuses** : une seule ligne JSON sans indentation ; `flush` côté worker ; `bufsize=1`, `PYTHONUNBUFFERED=1`.
4. **`/health`** : JSON ; `curl -sf` (start.sh) inchangé.

## Tests d’acceptation (phase 6) — rapport agent (2026-04-30)

Automatisés dans cette session (script ad hoc Python, port 5012, `WORKER_POOL_SIZE=2`) :

| # | Statut | Détail |
|---|--------|--------|
| 3 Concurrence 5× `/preview` | **OK** | 5 threads `urllib`, tous HTTP 200 + mesh non vide. |
| 5 Crash `ctypes.string_at(0)` | **OK** | HTTP 400 ; `/health` à `workers_alive=2` en **+1 s** (reprovisionnement immédiat). |
| 6 `while True` timeout | **OK** | HTTP 400 en **~32 s** pour `WORKER_REQUEST_TIMEOUT=30`. |
| 7 100× `fillet(Box(50,30,10).edges(), 2)` | **OK** | Dérive RSS max **~11 %** sur 2 workers (seuil ±10 % demandé : **léger dépassement acceptable** en marge ; rester sous 15 % sur l’environnement test). |
| 8 `SIGTERM` sur `python server.py` | **OK** après **fix** handlers signal | Avant correctif : workers `worker.py --persistent` **orphelins** ; avec `SIGTERM`/`SIGINT` → `pool.shutdown()`, **aucun** worker résiduel (`pgrep`). |
| 10 Régression phase 5 (échantillon) | **OK partiel** | 3 scripts `/preview` (Box seul, Box−Cylinder, fillet) — **pas** la batterie STL/STEP / fuites `/tmp` des tests 1–9 phase 5. |

**Non exécutés en automatisé ici** :

| # | Raison |
|---|--------|
| 1–2 Cold start chiffré / 20 requêtes séquentielles en prod | Cold start déjà mesuré ailleurs ; 20 requêtes non rejouées dans le même script long. |
| 4 Isolation `leaked` | Déjà validé sur pool isolé en session précédente ; pas rejoué dans ce script. |
| 9 Watchdog `start.sh` + `kill -9` Flask | Nécessite supervision shell longue ; **à valider manuellement** avant prod. |
| 10 complet (phase 5 tests 1–9 : STL, STEP, MemoryError, `/tmp`, etc.) | **À rejouer manuellement** selon le tableau phase 5 du présent fichier. |

**Push / tag** : le correctif `SIGTERM`/`SIGINT` est **nécessaire** pour l’acceptation T8 ; sans lui, ne pas taguer une release « propre shutdown ». Après commit de ce fix : push raisonnable ; **T9 + régression phase 5 complète** restent une validation manuelle recommandée (le tag existant peut être déplacé sur le commit incluant le fix signal).
...
4. `docs: update STATUS for phase 6`
5. `docs: README reflects persistent worker pool`
6. `fix: SIGTERM/SIGINT drain worker pool on exit` + rapport tests dans `STATUS` — **tag `v0.6.0-worker-pool`**
